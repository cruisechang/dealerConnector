...
func Test_historyResultType6Processor_Run(t *testing.T) {
	...
	for _, v := range rpcClients {
		...
		if err != nil {
			t.Fatalf("Test_historyResultType6Processor_Run rpc addClent error=%s", err.Error())
		}
	}

	p, _ := NewHistoryResultType6Processor(NewBasicProcessor(nx, conf, rpc))
	...
	obj := &[]config.HistoryResultType6CmdData{
		{},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err := p.Run(tt.args.obj); (err != nil) != tt.wantErr {
				t.Errorf("historyResultType6Processor.Run() error = %v, wantErr %v", err, tt.wantErr)
			}
		})
	}
}
