...
func Test_roundResultType0Processor_Run(t *testing.T) {
	...
	for _, v := range rpcClients {
		...
		if err != nil {
			t.Fatalf("Test_roundResultType0Processor_Run rpc addClent error=%s", err.Error())
		}
	}

	p, _ := NewRoundResultType0Processor(NewBasicProcessor(nx, conf,  rpc))
	...
	obj := &[]config.RoundResultType0CmdData{
		{},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if err := p.Run(tt.args.obj); (err != nil) != tt.wantErr {
				t.Errorf("Test_roundResultType0Processor_Run.Run() error = %v, wantErr %v", err, tt.wantErr)
			}
		})
	}
}
