...
func Test_roomLoginProcessor_Run(t *testing.T) {
	...
	for _, v := range rpcClients {
		...
		if err != nil {
			t.Fatalf("Test_roomLoginProcessor_Run rpc addClent error=%s", err.Error())
		}
	}

	lp, _ := NewRoomLoginProcessor(NewBasicProcessor(nx, conf, rpc))
	...
	obj := &[]config.RoomLoginCmdData{
		config.RoomLoginCmdData{
			RoomID: 2001,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if err := lp.Run(tt.args.obj); (err != nil) != tt.wantErr {
				t.Errorf("roomLoginProcessor.Run() error = %v, wantErr %v", err, tt.wantErr)
			}
		})
	}
}
