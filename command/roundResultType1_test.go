...
func Test_roundResultType1Processor_Run(t *testing.T) {
	...
	for _, v := range rpcClients {
		...
		if err != nil {
			t.Fatalf("Test_roundResultType1Processor_Run rpc addClent error=%s", err.Error())
		}
	}

	p, _ := NewRoundResultType1Processor(NewBasicProcessor(nx, conf, rpc))
	...
	obj := &[]config.RoundResultType1CmdData{
		config.RoundResultType1CmdData{},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if err := p.Run(tt.args.obj); (err != nil) != tt.wantErr {
				t.Errorf("roundResultType1Processor.Run() error = %v, wantErr %v", err, tt.wantErr)
			}
		})
	}
}
