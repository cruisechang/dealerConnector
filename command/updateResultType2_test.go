...
func Test_updateResultType2Processor_Run(t *testing.T) {
	...
	for _, v := range rpcClients {
		...
		if err != nil {
			t.Fatalf("Test_updateResultType2Processor_Run rpc addClent error=%s", err.Error())
		}
	}

	p, _ := NewUpdateResultType2Processor(NewBasicProcessor(nx, conf, rpc))
	...
	obj := &[]config.UpdateResultType2CmdData{
		config.UpdateResultType2CmdData{},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if err := p.Run(tt.args.obj); (err != nil) != tt.wantErr {
				t.Errorf("updateResultType2Processor.Run() error = %v, wantErr %v", err, tt.wantErr)
			}
		})
	}
}
