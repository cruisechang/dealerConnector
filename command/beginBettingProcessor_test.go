...
func Test_beginBettingProcessor_Run(t *testing.T) {
	...
	for _, v := range rpcClients {
		...
		if err != nil {
			t.Fatalf("Test_beginBettingProcessor_Run rpc addClent error=%s", err.Error())
		}
	}

	lp, _ := NewBeginBettingProcessor(NewBasicProcessor(nx, conf, rpc))
	...
	obj := &[]config.BeginBettingCmdData{
		config.BeginBettingCmdData{
			Boot:  1,
			Round: 333,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if err := lp.Run(tt.args.obj); (err != nil) != tt.wantErr {
				t.Errorf("responseTestProcessor.Run() error = %v, wantErr %v", err, tt.wantErr)
			}
		})
	}
}
