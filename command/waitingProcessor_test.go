...
func Test_waitingProcessor_Run(t *testing.T) {
	...
	for _, v := range rpcClients {
		...
		if err != nil {
			t.Fatalf("Test_waitingProcessor_Run rpc addClent error=%s", err.Error())
		}
	}

	p, _ := NewWaitingProcessor(NewBasicProcessor(nx, conf, rpc))
	...
	obj := &[]config.WaitingCmdData{
		config.WaitingCmdData{
			Boot:  1,
			Round: 1,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if err := p.Run(tt.args.obj); (err != nil) != tt.wantErr {
				t.Errorf("waitingProcessor.Run() error = %v, wantErr %v", err, tt.wantErr)
			}
		})
	}
}
