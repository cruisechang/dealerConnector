...
func Test_endBettingProcessor_Run(t *testing.T) {
	...
	for _, v := range rpcClients {
		...
		if err != nil {
			t.Fatalf("Test_endBettingProcessor_Run rpc addClent error=%s", err.Error())
		}
	}

	lp, _ := NewEndBettingProcessor(NewBasicProcessor(nx, conf, rpc))
	...
	obj := &[]config.EndBettingCmdData{
		config.EndBettingCmdData{},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err := lp.Run(tt.args.obj); (err != nil) != tt.wantErr {
				t.Errorf("endBettingProcessor.Run() error = %v, wantErr %v", err, tt.wantErr)
			}
		})
	}
}
