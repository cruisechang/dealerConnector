...
func Test_historyResultType0Processor_Run(t *testing.T) {
	...
	p, _ := NewHistoryResultType0Processor(NewBasicProcessor(nx, conf, rpc))
	...
	obj := &[]config.HistoryResultType0CmdData{
		config.HistoryResultType0CmdData{},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if err := p.Run(tt.args.obj); (err != nil) != tt.wantErr {
				t.Errorf("historyResultType0Processor.Run() error = %v, wantErr %v", err, tt.wantErr)
			}
		})
	}
}
