...
func Test_roundResultType6Processor_Run(t *testing.T) {
	...
	p, _ := NewRoundResultType6Processor(NewBasicProcessor(nx, conf, rpc))
	...
	obj := &[]config.RoundResultType6CmdData{
		{},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if err := p.Run(tt.args.obj); (err != nil) != tt.wantErr {
				t.Errorf("roundResultType6Processor.Run() error = %v, wantErr %v", err, tt.wantErr)
			}
		})
	}
}
