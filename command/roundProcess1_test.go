...
func Test_roundProcessType1Processor_Run(t *testing.T) {
	...
	for _, v := range rpcClients {
		...
		if err != nil {
			t.Fatalf("Test_roundProcessType1Processor_Run rpc addClent error=%s", err.Error())
		}
	}

	p, _ := NewRoundProcess1(NewBasicProcessor(nx, conf,  rpc))
	...
	obj := &[]config.RoundProcess1CmdData{
		config.RoundProcess1CmdData{},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err := p.Run(tt.args.obj); (err != nil) != tt.wantErr {
				t.Errorf("roundProcessType1Processor.Run() error = %v, wantErr %v", err, tt.wantErr)
			}
		})
	}
}
