...
func Test_updateResultType6Processor_Run(t *testing.T) {
	...
	for _, v := range rpcClients {
		...
		if err != nil {
			t.Fatalf("Test_updateResultType6Processor_Run rpc addClent error=%s", err.Error())
		}
	}

	p, _ := NewUpdateResultType6Processor(NewBasicProcessor(nx, conf,  rpc))
	...
	obj := &[]config.UpdateResultType6CmdData{
		{},
	}
