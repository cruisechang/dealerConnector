...
func Test_historyResultType2Processor_Run(t *testing.T) {
	...
	p, _ := NewHistoryResultType2Processor(NewBasicProcessor(nx, conf,  rpc))
	...
	obj := &[]config.HistoryResultType2CmdData{
		{},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if err := p.Run(tt.args.obj); (err != nil) != tt.wantErr {
				t.Errorf("historyResultType2Processor.Run() error = %v, wantErr %v", err, tt.wantErr)
			}
		})
	}
}
