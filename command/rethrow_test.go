...
func Test_rethrowProcessor_Run(t *testing.T) {
	...
	p, _ := NewRethrowProcessor(NewBasicProcessor(nx, conf, rpc))
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if err := p.Run(tt.args.obj); (err != nil) != tt.wantErr {
				t.Errorf("rethrowProcessor.Run() error = %v, wantErr %v", err, tt.wantErr)
			}
		})
	}
}
