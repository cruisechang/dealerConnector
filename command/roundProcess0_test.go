...
func Test_roundProcess0_Run(t *testing.T) {
	...
	for _, v := range rpcClients {
		...
		if err != nil {
			t.Fatalf("Test_roundProcessType0Processor_Run rpc addClent error=%s", err.Error())
		}
	}

	p, _ := NewRoundProcess0(NewBasicProcessor(nx, conf, rpc))
	...
	obj := &[]config.RoundProcess0CmdData{
		config.RoundProcess0CmdData{},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if err := p.Run(tt.args.obj); (err != nil) != tt.wantErr {
				t.Errorf("roundProcess0.Run() error = %v, wantErr %v", err, tt.wantErr)
			}
		})
	}
}
