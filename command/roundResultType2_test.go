...
func Test_roundResultType2Processor_Run(t *testing.T) {
	...
	for _, v := range rpcClients {
		...
		if err != nil {
			t.Fatalf("Test_roundResultType2Processor_Run rpc addClent error=%s", err.Error())
		}
	}

	p, _ := NewRoundResultType2Processor(NewBasicProcessor(nx, conf,  rpc))
	...
	obj := &[]config.RoundResultType2CmdData{
		{},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if err := p.Run(tt.args.obj); (err != nil) != tt.wantErr {
				t.Errorf("roundResultType2Processor.Run() error = %v, wantErr %v", err, tt.wantErr)
			}
		})
	}
}
