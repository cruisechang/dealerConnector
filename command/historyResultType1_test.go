...
func Test_getResultHistoryBADTProcessor_Run(t *testing.T) {
	...
	for _, v := range rpcClients {
		...
		if err != nil {
			t.Fatalf("Test_getResultHistoryBADTProcessor_Run rpc addClent error=%s", err.Error())
		}
	}

	p, _ := NewHistoryResultType1Processor(NewBasicProcessor(nx, conf, rpc))
	...
	obj := &[]config.HistoryResultType1CmdData{
		config.HistoryResultType1CmdData{},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if err := p.Run(tt.args.obj); (err != nil) != tt.wantErr {
				t.Errorf("getResultHistoryBADTProcessor.Run() error = %v, wantErr %v", err, tt.wantErr)
			}
		})
	}
}
