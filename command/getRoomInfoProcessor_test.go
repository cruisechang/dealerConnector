...
func Test_getRoomInfoProcessor_Run(t *testing.T) {
	...
	for _, v := range rpcClients {
		...
		if err != nil {
			t.Fatalf("Test_getRoomInfoProcessor_Run rpc addClent error=%s", err.Error())
		}
	}

	lp, _ := NewGetRoomInfoProcessor(NewBasicProcessor(nx, conf,  rpc))
	...
	obj := &[]config.GetRoomInfoCmdData{
		config.GetRoomInfoCmdData{},
	}
