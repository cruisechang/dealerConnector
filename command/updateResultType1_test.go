...
func Test_updateResultType1Processor_Run(t *testing.T) {
	...
	for _, v := range rpcClients {
		...
		if err != nil {
			t.Fatalf("Test_updateResultType1Processor_Run rpc addClent error=%s", err.Error())
		}
	}

	p, _ := NewUpdateResultType1Processor(NewBasicProcessor(nx, conf, rpc))
	...
	obj := &[]config.UpdateResultType1CmdData{
		config.UpdateResultType1CmdData{},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if err := p.Run(tt.args.obj); (err != nil) != tt.wantErr {
				t.Errorf("UpdateResultType1CmdData.Run() error = %v, wantErr %v", err, tt.wantErr)
			}
		})
	}
}
