...
/*
func userKill(sessionID string){


	n, err := nex.NewNex("/Users/cruise/go/bin/nexConfig.json")
	if err != nil {
		return
	}
	nexInstance:=n

	conf,_:=NewConfigurer("slotConfig.json")
	basic:=NewBasicProcessor(nexInstance,conf)
	...
	postRes,postErr:=basic.BackstageRequest("gameUserKill",bp)

	if postErr != nil {
		fmt.Errorf("test login error:%v",postErr)
		return
	}

	fmt.Printf("TestUserKill postRes:%s\n",postRes)
}
*/
